...
func _init() -> void:
	...
	var nord := [SPIEL_DOSEN, DEKO_ENTEN, SPIEL_SCHIESS, DEKO_SUESS, SPIEL_LUKAS, DEKO_DREH]
	var i_n := 0
	for z: float in [64.0, 71.5, 79.0]:
		for seite: float in [-1.0, 1.0]:
			_stand(buden, nord[i_n % nord.size()], Vector3(seite * 11.5, 0, z), Vector3(-seite, 0, 0))
			i_n += 1
	var sued := [SPIEL_SCHIESS, DEKO_DREH, DEKO_SUESS, SPIEL_DOSEN, DEKO_ENTEN, SPIEL_LUKAS]
	var i_s := 0
	...
	var ringbuden := [[60.0, SPIEL_DOSEN], [120.0, DEKO_ENTEN], [150.0, SPIEL_LUKAS], [210.0, DEKO_SUESS],
