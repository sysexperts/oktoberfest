...
class Lauf extends Node:
	const ARTEN := ["ringwurf", "entenangeln", "gluecksrad", "stemmen", "nagelbalken", "kegeln"]
	...
	func _ready() -> void:
		...
		for art: String in ARTEN:
			...
			kamera.global_position = bude.global_position + vorn * 9.5 + rechts * 2.6 + Vector3(0, 3.0, 0)
			kamera.look_at(bude.global_position + vorn * 0.5 + Vector3(0, 1.7, 0))
			for i in 12:
				await get_tree().process_frame
